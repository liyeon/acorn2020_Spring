...
public class FileDownView extends AbstractView{
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		//ModelAndView 객체에 .addObject() 해서 담은 객체는 model(로 전달이 됨)에 들어있다.
		...
		//jsp에서 application은 사용 할 수 있지만 java에서는 불가하기 때문에 request.getServletContext()로 받아온다
		String path = request.getServletContext().getRealPath("/upload") + File.separator + saveFileName; //경로를 구성헤준다. 우리는 경로를 모르니깐
		//System.out.println(path);
		//업러드 폴더에 어떤 이름으로 저장되어있는지
		...
		response.setContentLengthLong(dto.getFileSize()); //파일사이즈
